...
class TestTradingViewMCPTransport(unittest.TestCase):
    ...
    def test_http_transport(self):
        ...
        try:
            ...
            pine = f.run_pinescript(
                script="//@version=5\nindicator('x')\nplot(close)",
                symbol="XAUUSD",
                timeframe="15",
            )
            self.assertIsNotNone(pine)
            self.assertTrue(pine.get("ok"))
            self.assertEqual(pine.get("symbol"), "XAUUSD")
        finally:
            server.shutdown()
            server.server_close()

    def test_command_transport_run_pinescript(self):
        os.environ["TICK_COLLECTOR_SOURCE"] = "tradingview_mcp"
        os.environ["TRADINGVIEW_MCP_TRANSPORT"] = "command"
        ...
        self.assertEqual(out.get("symbol"), "EURUSD")
